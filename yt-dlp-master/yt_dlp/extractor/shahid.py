...
class ShahidBaseIE(AWSIE):
    ...
    _AWS_API_KEY = os.environ.get('SHAHID_AWS_API_KEY', 'not-a-real-api-key')
    ...
    def _call_api(self, path, video_id, request=None):
        ...
        try:
            return self._aws_execute_api({
                'uri': '/proxy/v2/' + path,
                'access_key': os.environ.get('SHAHID_AWS_ACCESS_KEY_ID', 'not-a-real-access-key'),
                'secret_key': os.environ.get('SHAHID_AWS_SECRET_ACCESS_KEY', 'not-a-real-secret-key'),
            }, video_id, query)
